...
for i = 1:sim_time
    cost = @(joint_angles) 0.5*([foot_trajectory(:,i)] - [xe(kinematics3D(joint_angles,param))]).'*eye(3)...     
                              *([foot_trajectory(:,i)] - [xe(kinematics3D(joint_angles,param))])...                           
                           + 0.5*trace(rotmZYX-rotation(kinematics3D(joint_angles,param)))...
                            + 0.0001*norm(joints0-joint_angles);
                           
                      %                            + 0.5*trace(eye(3)-RL(kinematics3D(joint_angles,param))) ...
%                             + 0.5*trace(eye(3)-RR(kinematics3D(joint_angles,param))) + ...
    nlconstraint = @(joint_angles) nonlconFoot(joint_angles,param,joints0);
