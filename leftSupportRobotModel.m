function output = leftSupportRobotModel
leftSupportRobot = rigidBodyTree('DataFormat','column');
...
transform = inv([[eulerRotation([0 0 -1.57079]);[0,0,0]],[0;0;0;1]]);
setFixedTransform(joint,transform);
joint.JointAxis = [0 -1 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'base');
...
transform = inv([[eulerRotation([-3.14159  0      -3.14159]);[0,0,0]],[0; 0.04;  0;1]]);
setFixedTransform(joint,transform);
joint.JointAxis = [-1 0 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'left_foot');
...
transform = inv([[eulerRotation([3.14159 0 -3.14159]);[0,0,0]],[0;0.01;-0.2;1]]);
setFixedTransform(joint,transform);
joint.JointAxis = [1 0 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'left_ankle');
...
transform = inv([[eulerRotation([0.05 0 0]);[0,0,0]],[0;0.00949;-0.18976;1]]);
setFixedTransform(joint,transform);
joint.JointAxis = [1 0 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'left_lower_leg');
...
transform = inv([[eulerRotation([-0.05 0 0]);[0,0,0]],[0;-0.01;0;1]]);
setFixedTransform(joint,transform);
joint.JointAxis = [0 -1 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'left_upper_leg');
...
body.Joint = joint;
addBody(leftSupportRobot, body, 'left_hip_roll');

%Torso
body = rigidBody('torso');
joint = rigidBodyJoint('torso', 'fixed');
transform = inv([[eulerRotation([0 0 1.57079]);[0,0,0]],[0;0.055;0;1]]);
setFixedTransform(joint,transform);
body.Joint = joint;
addBody(leftSupportRobot, body, 'left_hip_yaw');

%right_hip_yaw
body = rigidBody('right_hip_yaw');
joint = rigidBodyJoint('right_hip_yaw', 'revolute');
transform = [[eulerRotation([0,0,pi/2]);[0,0,0]],[0;-0.055;0;1]];
setFixedTransform(joint,transform);
joint.JointAxis = [0 0 1];
body.Joint = joint;
addBody(leftSupportRobot, body, 'torso');
...
joint.JointAxis = [0 -1 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'right_hip_yaw');        
%right_upper_leg
...
transform = [[eulerRotation([-0.05,0,0]);[0,0,0]],[0;-0.01;0.01;1]];
setFixedTransform(joint,transform);
joint.JointAxis = [1 0 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'right_hip_roll');
...
transform = [[eulerRotation([0.05,0,0]);[0,0,0]],[0;0.00999;-0.19975;1]];
setFixedTransform(joint,transform);
joint.JointAxis = [1 0 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'right_upper_leg');
...
transform = [[eulerRotation([pi,0,-pi]);[0,0,0]],[0;0.01;-0.2;1]];
setFixedTransform(joint,transform);
joint.JointAxis = [-1 0 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'right_lower_leg');
%right_ankle_roll
...
transform = [[eulerRotation([-pi,0,-pi]);[0,0,0]],[0;    0.04;  0;1]];
setFixedTransform(joint,transform);
joint.JointAxis = [0 -1 0];
body.Joint = joint;
addBody(leftSupportRobot, body, 'right_ankle');

%head
body = rigidBody('head');
joint = rigidBodyJoint('head_pitch', 'revolute');
body.Mass = 2.539;
...
body.Joint = joint;
addBody(leftSupportRobot, body, 'torso');
show(leftSupportRobot)
getTransform(leftSupportRobot,homeConfiguration(leftSupportRobot),'right_foot')
output = leftSupportRobot;
end